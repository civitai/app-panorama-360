...
describe('mergeEnvFile', () => {
  ...
  it('REPLACES existing assignments in place and PRESERVES other lines + comments', () => {
    const existing = [
      '# my env',
      'VITE_BLOCK_ALLOWED_PARENT_ORIGINS=http://localhost:5187',
    ...
    expect(out).toContain('VITE_BLOCK_ALLOWED_PARENT_ORIGINS=http://localhost:5187');
...
describe('runDevLiveSetup (the orchestration the middleware calls)', () => {
  ...
  it('on a successful mint writes BOTH env vars (merging, preserving others)', async () => {
    const fs = memFs({
      ...
      [envPath]: 'VITE_BLOCK_ALLOWED_PARENT_ORIGINS=http://localhost:5187\n',
    });
    ...
    expect(written).toContain('VITE_BLOCK_ALLOWED_PARENT_ORIGINS=http://localhost:5187');
  });
